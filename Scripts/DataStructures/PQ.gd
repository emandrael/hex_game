...
# Version: 1.0.4
# ATTENTION! The code below assumes this script to be autoloaded as "DS"-named object! Search for "DS." to find the offending lines of code
...
class PriorityQueues:
	...
	func get_item_priority(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): get_item_priority(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return false
	...
	func insert(priority, value):
		...
		if (typeof(priority) != TYPE_INT && typeof(priority) != TYPE_FLOAT):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): Priority of incorrect type (" + str(typeof(priority)) + ") (should be float or int)! Aborting insert()")
			return false
	...
	func remove_root(return_array = false):
		...
		if (isEmpty):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): Can't delete root in empty heap! isEmpty = true, currentSize = " + str(currentSize))
			return null
	...
	func change_priority(arrayIdx, new_priority):
		...
		if (arrayIdx < 0 || arrayIdx + 1 > currentSize):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): Can't access array at index '" + str(arrayIdx) + "', out of bounds!")
			return false
		
		if (typeof(new_priority) != TYPE_INT && typeof(new_priority) != TYPE_FLOAT):
			
			# Invalid priority parameter. Abort
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): Can't assign priority '" + str(new_priority) + "', NaN!")
			return false
	...
	func remove(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(get_instance_id()) + "): remove(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return null
